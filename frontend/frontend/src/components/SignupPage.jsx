...
export default function SignupPage({ onLogin }) {
  ...
  async function submit(event) {
    ...
    try {
      const { user } = await signup({ name: form.name.trim(), email: form.email.trim().toLowerCase(), password: form.password, role });
      onLogin?.(user);
  ...
  return <div className="auth-page signup-page">
    <header className="signup-nav"><Link to="/" className="ref-brand"><span className="brand-mark">♜</span> AEGIS</Link></header>
    <main className="auth-shell signup-shell">
      ...
      <h1 className="auth-heading">Create your account</h1><p className="auth-sub">One account for the senior, one for the caregiver.</p>
      <form onSubmit={submit} noValidate>
        ...
        {error && <div className="error-box">⚠ {error}</div>}<button className="auth-submit" type="submit" disabled={loading || !form.name || !form.email || !form.password}>{loading ? 'Creating account…' : 'Create Account'}</button>
      </form>
