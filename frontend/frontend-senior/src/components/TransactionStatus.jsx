...
/**
 * TransactionStatus — now polls the backend instead of reading from a cookie.
 *
 * Props from App.jsx:
 *   transaction.txId            — backend tx_id (integer)
 *   transaction.amount          — numeric amount
 *   transaction.beneficiary     — payee display name
 *   transaction.riskReasons     — string[]
 *   transaction.status          — backend status (PENDING_APPROVAL | APPROVED | BLOCKED)
 *   transaction.coolingOffExpiry — ISO string or null
 *   transaction.transactionType — 'transfer' | 'fd_break'
 */
function TransactionStatus({ transaction, onDone }) {
  ...
  // ── Poll backend every 2 s for status changes ────────────────────────────
  useEffect(() => {
    if (status !== 'pending_verification') return;
    if (!txId) return;
